...
from ai.models import MotionType
...
class CourseListCreateAPI(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        # Course 생성
        course = serializer.save(company=self.request.user)

        # 연관 MotionType 자동 생성
        MotionType.objects.update_or_create(
            motion_name=course.title,
            defaults={'description': f"'{course.title}' 과정용 동작"}
        )
