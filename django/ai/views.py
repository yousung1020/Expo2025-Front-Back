...
# --- Serializers ---
from .serializers import EvaluationRequestSerializer, MotionSerializer, MotionTypeSerializer, UserRecordingSerializer
...
class UserCourseRecordingView(APIView):
    def get(self, request, emp_no, course_id, *args, **kwargs):
        try:
            course = Course.objects.get(pk=course_id)
            motion_type = MotionType.objects.get(motion_name=course.title)
            
            user_recording = UserRecording.objects.filter(
                user__emp_no=emp_no,
                motion_type=motion_type
            ).order_by('-recorded_at').first()

            if not user_recording:
                return Response({"detail": "해당 과정에 대한 평가 기록을 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND)

            serializer = UserRecordingSerializer(user_recording)
            return Response(serializer.data, status=status.HTTP_200_OK)

        except Course.DoesNotExist:
            return Response({"detail": "과정을 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND)
        except MotionType.DoesNotExist:
            return Response({"detail": "해당 과정에 연결된 동작 유형을 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND)
